...
class TestCreditNoteDownload:
    async def test_download_pdf(
        ...
        """GET /v2/credit-notes/{id}/pdf -> binary."""
        mock_factus.get.return_value = httpx.Response(
            200,
        ...
        response = await service.download_pdf(67890)

        mock_factus.get.assert_awaited_once_with(
            "/v2/credit-notes/67890/pdf"
        )
    ...
    async def test_download_xml(
        ...
        """GET /v2/credit-notes/{id}/xml -> XML."""
        mock_factus.get.return_value = httpx.Response(
            200,
            content=b'<?xml version="1.0" encoding="UTF-8"?><credit_note>...</credit_note>',
            headers={"Content-Type": "application/xml"},
        )

        response = await service.download_xml(67890)

        mock_factus.get.assert_awaited_once_with(
            "/v2/credit-notes/67890/xml"
        )
        assert response.status_code == 200
